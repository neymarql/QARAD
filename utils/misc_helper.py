...
class EarlyStopping:
    def __init__(self, patience=10, delta=0.0, larger_better=True, rank=0):
        self.patience = patience
        self.delta = delta
        self.best = -float('inf') if larger_better else float('inf')
    ...
    def step(self, metric):
        improved = (metric > self.best + self.delta) if self.larger_better else (metric < self.best - self.delta)
        if improved:
            self.best = metric
            self.num_bad_epochs = 0
            if self.rank == 0:
                print(f"EarlyStopping: Metric improved to {self.best:.5f}, resetting bad epochs counter.")
        else:
            self.num_bad_epochs += 1
            if self.num_bad_epochs >= self.patience:
                self.stop = True
            if self.rank == 0:
                print(f"EarlyStopping: {self.num_bad_epochs}/{self.patience} bad epochs, "
                      f"current metric: {metric:.5f}, best metric: {self.best:.5f}")
        return improved
